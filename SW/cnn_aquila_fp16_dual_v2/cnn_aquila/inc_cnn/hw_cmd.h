...
void set_dram_read_bn_mul_weight_cmd() {
    *WRITE_DATA_TYPE_ADDR = 2;
    __asm__ volatile ("nop");
}

void set_dram_read_bn_add_weight_cmd() {
    *WRITE_DATA_TYPE_ADDR = 3;
    __asm__ volatile ("nop");
}
...
void read_conv_weight_cmd(int length, int depth, uint32_t addr) {
    set_dram_read_weight_cmd();
    reset_sram_offset_cmd();
    set_length_cmd(length);

    set_read_rounds_cmd(depth);
    set_read_offset_cmd(length * 2);
    set_addr_cmd(addr);
    trigger_dram_read_cmd();
    wait_idle_quick_cmd();
...
void read_batchNorm_weight_cmd(int length, uint32_t addr_1, uint32_t addr_2) {
    // read batchNorm weight 1
    set_read_rounds_cmd(1);
    set_dram_read_bn_mul_weight_cmd();
    set_length_cmd(length);
    set_addr_cmd(addr_1);
    trigger_dram_read_cmd();
    wait_idle_quick_cmd();
